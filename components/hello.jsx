...
function MainApp({ username, credentialHash }) {
    ...
    const fetchDecks = async () => {
      ...
      const handleDeckFormSubmit = async (e) => {
        ...
        await fetch('/api/decks', {
          ...
          body: JSON.stringify({ name: deckName, auth: { username, credentialHash } }),
        });
        setDeckName('');
      ...
      const handleFlashcardFormSubmit = async (e) => {
        ...
        await fetch(`/api/decks/${currentDeckId}/cards`, {
          ...
          body: JSON.stringify({ front: cardFront, back: cardBack, auth: { username, credentialHash } }),
        });
        setCardFront('');
        setCardBack('');
    ...
    return <>
        ...
        <div className="container">
            ...
            <select id="decks" value={currentDeckId || '0'} onChange={handleDeckChange}>
  ...
  return (
    <div>
      ...
      <main>
        ...
        {credentialHash && <MainApp username={username} credentialHash={credentialHash} />}
      </main>
    </div>
  );
}
